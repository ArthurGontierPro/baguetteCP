...
| M1-T35 | A `--time` flag (or a library-level harness) so solve time is separable from process start-up | TODO | From M3-T5. The benchmark measures the CLI, and on 15 of 18 models process start-up swallows the entire signal — every timing there is `exec`. Until this exists, **no timing claim about the solver can be made from the model suite**, in either direction. This is the precondition for M3-T5 being able to answer anything about speed, including D-0026's prediction |
