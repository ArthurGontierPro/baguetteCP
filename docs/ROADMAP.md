...
| M1-T22 | `del range` is half-open: fix `Writer.wipe_level`, correct PROOF-FORMAT §5 | TODO | **Verified bug.** veripb 3.0.2 deletes `[LO, HI)`; we emit `del range lo hi` for the *inclusive* run, so every multi-id run leaves its last id live in the checker while `tags`/`live` drop it (I-X3). 7 lines across 5 of 14 models today. Not unsound — proof growth. See `docs/GCS-COMPARISON.md` §2.1 |
| M1-T23 | Checked arithmetic in `Linear`/`Ne`; cap declared domains at compile time | TODO | **The only soundness gap found.** No overflow handling exists anywhere in `lib/`; a wrapped product prunes wrongly *and* writes the `.opb` row from the same wrapped arithmetic, so veripb accepts it. Add an overflow axis to `test_matrix.ml`. Independent — good parallel task |
| M1-T24 | Trail-cursor walk in `Engine.propagate`, replacing `Store.trail_entries` | TODO | `engine.ml:60` allocates the whole trail per propagator call and keeps only the newest `n_new`; `Store.trail_entry` already gives O(1) indexed access and its own comment says so. Pure perf, no invariant touched |
...
| M1-T27 | Decision record: bound-fact chains are width-proportional; state the policy | TODO | `order_reason.ml:38-43` builds one literal per value between declared and current bound (D-0010 requires the chain); D-0005's hole-span cap does not cover it. Doc-only now; the interval restatement is M4 work |
...
| M2-T0 | **Amend D-0011**: D-0015's `Model_row` closed its ADT gap, so multi-row derivations are permitted | TODO | A literal reading of D-0011 forbids writing a Hall-interval justification at all, and GCS's cites one line per Hall value *and* per Hall variable. Decision record only, no code. **Must land before M4-T1** |
