...
| M7-T10 | agent-harness | 2026-09-23 | **The torn `.fzn` was an ID COLLISION, not a non-atomic write.** The node harness keyed on `<year>_<family>`, but a family directory holds many `.mzn` files -- `2010/bacp/` holds fifteen -- so up to fifteen concurrent jobs flattened to the SAME path, read each other's bytes and `rm`'d it from under one another. All 26 suspect rows (16 torn parses, 5 "no such file", 4 "no solve item", 1 `End_of_file`) belong to one of **18 colliding ids**; so do 101 rows in total, and D-0068's "436 instances" was really **353 distinct ids**. Re-run atomically: **2010_bacp is 8 OK-PROOF-VERIFIED + 7 TIMEOUT, not 15 refusals**; corpus-wide verified proofs **8 -> 17**. The other suspects are genuine, in their proper buckets. |
| M7-T4 | agent-harness | 2026-09-23 | **`scripts/corpus_run.sh`** + **`scripts/corpus_selftest.sh`**: the corpus harness, checked in. flatten -> solve -> **veripb 3.0.2** -> classify, parameterised by `CORPUS`/`OUT`/`PAR`/`MEM_KB`/timeouts, `PAR` capped at 92 and `MEM_KB` at 32 GB, flattening through `tools/baguette.msc` + `mznlib/` (D-0067), resumable by id, `DONE-<epoch>` marker, `--report` that refuses to print a total for a marker-less run. Eleven buckets; `PROOF-REJECTED` is its own and keeps its artefacts. **`validate_fzn` gates every solver-side bucket** -- D-0069's rule made structural. |
